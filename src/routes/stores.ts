...
export const dinnersToday = derived(dinners, ($dinners) => {
	...
	return $dinners.filter((d: any) => DateTime.fromISO(d).toFormat('yyyy-MM-dd') === today).length;
});
...
export const timeFromLastDinner = derived(dinners, ($dinners, set) => {
	const calc = () => {
		const lastDinner = DateTime.fromISO($dinners[0]);
		const x = lastDinner.isValid
			? DateTime.now().diff(lastDinner, ['hours', 'minutes', 'seconds']).toFormat('hh:mm:ss')
			: 'N/A';
		set(x);
	};
	calc();
...
export const timeFromLastNap = derived(naps, ($naps, set) => {
	const calc = () => {
		...
		const x = lastNap.isValid
			? DateTime.now().diff(lastNap, ['hours', 'minutes', 'seconds']).toFormat('hh:mm:ss')
			: 'N/A';
		set(x);
	};
	calc();
